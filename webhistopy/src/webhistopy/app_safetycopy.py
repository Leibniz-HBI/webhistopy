...
class WebhistoPy(toga.App):
    ...
    def startup(self):
        ...
        self.main_window = toga.MainWindow(
            size=(500, 500), position=(25, 25), title=self.formal_name
        )
    ...
    async def selectfile(self,widget):
        ...
        self.visualization.add(
            toga.Button("Create and visualize network", style=large_font, on_press=self.create_networks)
        )
    ...
    def top30(self,button):
        ...
        #plt.show()
        home = expanduser("~")
        plt.savefig(str(Path(home).joinpath("Desktop","example.svg")))
        print("done")
        f.close()
